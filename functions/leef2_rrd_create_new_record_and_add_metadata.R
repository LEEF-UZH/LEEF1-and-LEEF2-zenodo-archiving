...
#'
#' @examples
rrd_create_new_record_and_add_metadata <- function(zenodo)
  {
  
  
  metadata_json <- jsonlite::read_json("metadata_LEEF2_sampledate_template.json")
  metadata_json <- metadata_json[grep("^_", names(metadata_json), invert = TRUE)]

  metadata_json$title <- paste0("LEEF2 level3 data (one of four)")
