---
title: "Archiving data to zenodo -- LEEF2"
author: "Owen Petchey"
format: html
editor: visual
---
...
Tests show that one sampling date of level1 and level2 data gives less than 50 GB of data.We will make a zenodo record for each sampling date. A sampling date record will contain both level1 and level2 data.
...
The bibliographic metadata will be used to populate fields of the zenodo record, such as title, authors, and keywords. The bibliographic metadata is stored in the file `metadata_LEEF2_sampledate_template.json` and can be read in with:

```{r}
bib_metadata <- jsonlite::read_json("metadata_LEEF2_sampledate_template.json")
...
```{r}
archive_location <- "/Volumes/LEEF-2_archive/archive"
...
We see that there are 120 sampling dates for most data types, but fewer for some data types.
...
```{r}
my_zenodo_records <- zenodo$getDepositions(q = "title:(LEEF2 AND data)",
                                           size = 200)
...
```{r}
timestamps_not_done <- all_timestamps
